...
public:
  CUDACallNode(const int &row, const int &col) : Node(row, col) {}
  CUDACallNode(const int &row, const int &col,
  ...
  void addConfig(const shared_ptr<Node> &nd) {
    if (config_ == nullptr) {
      config_ = shared_ptr<CompoundNode>(new CompoundNode(row_, col_));
