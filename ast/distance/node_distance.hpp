...
template <typename T> int node_distance(const T &s, const T &t) { return 0; }
...
template <> int node_distance<RealNode>(const RealNode &s, const RealNode &t) {
  return 0;
}
...
template <>
int node_distance<IdentifierNode>(const IdentifierNode &s,
                                  const IdentifierNode &t) {
  return levenshtein_distance(s.getName(), t.getName());
}

template <typename T1, typename T2>
std::enable_if<std::is_same<T1, T2>::value, int> node_distance(const T1 &s,
                                                               const T2 &t) {
